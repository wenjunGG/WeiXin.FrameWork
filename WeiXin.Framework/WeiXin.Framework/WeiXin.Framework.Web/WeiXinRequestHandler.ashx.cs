                ...
                switch (httpMethod)
                {
                    ...
                    case "POST":
                        ...
                        else
                        {
                            ...
                            string signature1 = context.Request.QueryString["signature"], // 微信加密签名
                              timestamp1 = context.Request.QueryString["timestamp"], // 时间戳
                              nonce1 = context.Request.QueryString["nonce"]; // 随机数
                            
                            // 微信请求参数非空验证
                            if (!String.IsNullOrEmpty(signature1) && !String.IsNullOrEmpty(timestamp1) && !String.IsNullOrEmpty(nonce1))
                            {
                                // 检查加密签名是否正确
                                if (UtilityHelper.CheckSignature(signature1, timestamp1, nonce1))
                                {
                                    LogHelper.WriteLog("检查通过");
                                    Stream stream = context.Request.InputStream;
                                    // 解析微信请求
                                    XElement requestXml = XElement.Load(stream);
                                    // 处理微信消息请求，并返回信息
                                    WeiXinApplication weiXinApplication = new WeiXinApplication(requestXml);
                                    responseXml = weiXinApplication.GetResponseXml();
                                }
